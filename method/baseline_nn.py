...
class BaselineNeuralNet(Process):
    ...
    def run_training(self,model, train_loader, valid_loader, optimizer, criterion,  device, n_epochs=10):
        ...
        for epoch in range(n_epochs):
            ...
            train_loss = self.train_one_epoch(model, train_loader, optimizer, criterion, device)
            valid_loss, valid_preds, valid_targets = self.validate_one_epoch(model, valid_loader, criterion, device)
    ...
    def run(self):
        ...
        cat_features = ["item_id", "dept_name", "class_name", "subclass_name", "item_type", "store_id"]
        

        testandtrain = pd.concat([self.combined_sales, test_processed], axis=0, ignore_index=True)
        ...
        num_features = [col for col in self.combined_sales.columns if col not in exclude_cols and col not in cat_features]

        X_train, X_valid, X_test, y_train, y_valid = self.partition_data(self.combined_sales, test_processed, num_features+cat_features)
        ...
        cat_cardinalities = []
        for c in cat_features:
            max_val = max(X_train_scaled[c].max(), X_valid_scaled[c].max(), X_test_scaled[c].max())
        ...
        train_dataset = MLZoomCampSalesDataset(X_train_scaled, num_features, cat_features, target=y_train)
        valid_dataset = MLZoomCampSalesDataset(X_valid_scaled, num_features, cat_features, target=y_valid)
        ...
        model = BaselineEmbeddingNN(cat_cardinalities=cat_cardinalities,
        ...
        model, best_rmse = self.run_training(model, train_loader, valid_loader, optimizer, criterion, device)
